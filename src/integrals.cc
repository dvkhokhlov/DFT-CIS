...
std::vector<Matrix> compute_2body_fock_like_batch_s(const std::vector<Shell>& shells,
  ...
  std::vector<Matrix> batch_G(batch_P.size());
	...
	for(size_t b_i = 0, b_i_max = batch_P.size(); b_i < b_i_max; ++b_i){
		batch_G[b_i] = Matrix::Zero(n,n);
	}
              ...
              for(auto f3=0; f3!=n3; ++f3) {
                ...
                for(auto f4=0; f4!=n4; ++f4, ++f1234) {
                  const auto bf4 = f4 + bf4_first;
                  	...
                  	for(size_t b_i = 0, b_i_max = batch_P.size(); b_i < b_i_max; ++b_i){
						batch_G[b_i](bf1,bf2) += batch_P[b_i](bf3,bf4) * 2.0 * buf_1234[f1234];
					}
              ...
              for(auto f2=0; f2!=n2; ++f2) {
                ...
                for(auto f4=0; f4!=n4; ++f4, ++f1324) {
                  const auto bf4 = f4 + bf4_first;
                    ...
                    for(size_t b_i = 0, b_i_max = batch_P.size(); b_i < b_i_max; ++b_i){
						batch_G[b_i](bf1,bf2) -= batch_P[b_i](bf3,bf4) * buf_1324[f1324];
					}
                }
              }
            }
